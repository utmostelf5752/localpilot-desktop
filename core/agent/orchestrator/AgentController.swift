    ...
    private func runAgentLoop(task: String) async {
        ...
        do {
            ...
            for step in 1...20 {
                ...
                fakeCursorPosition = CGPoint(x: 260 + CGFloat(step * 18), y: 220 + CGFloat(step * 10))
                log(event: "planning", detail: "Requesting one action")

                let action = try await planner.proposeOneAction(originalTask: task, context: context, recentMessages: messages)
                ...
                let policy = policyEngine.classifyBatch(actions: [action], context: context)
            ...
            blockTask(reason: "Planner reached the 20 action safety limit.", action: nil)
